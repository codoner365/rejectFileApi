...
# 원본 엑셀 파일 경로
origin_excel_path = r"C:\cji_d\apiTest\design"
target_param = "출원(국제등록)번호"
...
def load_access_key():
    ...
    try:
        ...
        print("⚠️ access_key.txt 파일을 찾을 수 없습니다.")
        return None
...
def request_rejection_reason_parsing(xml_text):
    ...
    for idx, reject_info in enumerate(rejection_info_list, 1):  # 번호 매기기
        title = reject_info.find("rejectionContentTitle").text if reject_info.find("rejectionContentTitle") is not None else ""
        ...
        for detail in details:
            ...
            print("detail: " + detail_text)
            
            # HTML 엔터티 및 개행 문자 변환
            detail_text = re.sub(r"<BR>|BR&gt;", "\n", detail_text, flags=re.IGNORECASE)  # <BR> → 개행 문자로 변환
            detail_text = re.sub(r"&lt;", "<", detail_text)  # HTML 엔터티 변환
            detail_text = re.sub(r"&gt;", ">", detail_text)
            detail_text = detail_text.strip()

            # 제목과 상세 내용 형식으로 저장
            reason_texts.append(f"< {idx} 번째 의견통지서 >\n{title}\n{detail_text}" if title else f"< 거 절 이 유 {idx} >\n{detail_text}")
